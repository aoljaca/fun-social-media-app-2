...
export const Post = (props) => {
    ...
    for (let i=0; i < comments.length; i++) {
        ...
        items.push(<div className="comment" key={comment._id}>
            ...
            <button type="button" className="btn-small btn comment_trash red darken-3" onClick={() => handleDelete(comment._id)}><i className="fa fa-trash" aria-hidden="true"></i></button>
        </div>)
    }
    ...
    async function handleFollow() {
        const follow_user = post.user._id
        await fetch('/follow', {
            method: 'POST',
            body: JSON.stringify({ follow_user, user_id }),
            headers: { 'Content-Type': 'application/json' },
        })
        .then(res => res.json())
        .then(response => {
            if (response.message === 'white') {
                ...
                const newArray = user.following.filter(i => i !== follow_user)
                user.following = newArray
                localStorage.setItem('user', JSON.stringify(JSON.stringify(user)))
                props.update()
                props.setRefresh(props.refresh === 'refresh' ? '' : 'refresh')
            }
    ...
    if (post_user === user_id) {
        button = <button type="button" className="btn-small btn comment_trash red darken-3" onClick={handleDeletePost}><i className="fa fa-trash" aria-hidden="true"></i></button>
    }
            ...
            <div className="col s12 m6 offset-m3 l4 offset-l4">
                <div className="card grey darken-4 darken-1">
                    <div className="card-content white-text">
                        ...
                        <button className={button_class} onClick={handleFollow}>{following}</button>
                    </div>
                    <div className="card-action">
                        ...
                        <button type="button" className="btn-small btn i" onClick={handleComment}>Comment</button>
                    </div>
                    <div className="card-action">
                        {items}
                    </div>
